...
class Game(object):
	...
	def update(self): # Game Loop - Update
		...
		self.enemy.pos.x, self.enemy.pos.y = multyplayer.loop([self.player.pos.x, self.player.pos.y])
		...
		if self.player.rect.top <= HEIGHT / 4: # y
			for s in self.all_sprites:
				s.rect.y += abs(self.player.vel.y + 0.5 * self.player.acc.y)
			for e in self.entitys:
				e.pos.y += abs(self.player.vel.y + 0.5 * self.player.acc.y)
			for ef in self.player.master.effects:
				for p in ef[0].particles:
					p[0][1] += abs(self.player.vel.y + 0.5 * self.player.acc.y)
		...
		if self.player.rect.left <= WIDTH / 4: # x
			for s in self.all_sprites:
				s.rect.x += abs(self.player.vel.x + 0.5 * self.player.acc.x)
			for e in self.entitys:
				e.pos.x += abs(self.player.vel.x + 0.5 * self.player.acc.x)
			for ef in self.player.master.effects:
				for p in ef[0].particles:
					p[0][0] += abs(self.player.vel.x + 0.5 * self.player.acc.x)
